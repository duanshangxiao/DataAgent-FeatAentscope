...
3. 启动 Elasticsearch，确认集群至少达到 yellow。
...
- 默认向量库是 Elasticsearch，而主 Compose 没有包含 Elasticsearch 服务。
- `docker-compose-es.yml` 使用开发者机器上的绝对挂载路径，其他机器使用前必须调整。
...
- MySQL 管理库。
- Elasticsearch 数据或可重新生成的数据源与重建步骤。
- 使用本地文件存储时的上传目录。
...
- MySQL、Elasticsearch、上传目录已有备份与恢复演练。
